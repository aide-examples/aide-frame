#!/bin/bash
# LOC Statistics Generator for AIDE projects
#
# Usage:
#   loc-stats.sh <project-root> [--markdown]                    # Project-wide stats
#   loc-stats.sh <project-root> [--markdown] --system <name>    # System-specific stats
#
# Example:
#   aide-frame/tools/loc-stats.sh /home/user/aide-rap --markdown --system irma
...
if [ -z "$PROJECT_ROOT" ]; then
    echo "Usage: loc-stats.sh <project-root> [--markdown] [--system <name>]" >&2
    echo "  <project-root>  Path to the AIDE RAP project directory" >&2
    exit 1
fi
...
if [ ! -d "$PROJECT_ROOT/app" ]; then
    echo "Error: '$PROJECT_ROOT' does not look like an AIDE RAP project (no app/ directory)" >&2
    exit 1
fi

# Derive FRAME_ROOT from script location (this script lives in aide-frame/tools/)
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FRAME_ROOT="$(dirname "$SCRIPT_DIR")"
